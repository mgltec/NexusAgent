...
@Component({
  selector: 'app-PlayerHistoryAdvanced',
  templateUrl: './PlayerHistoryAdvanced.component.html',
  styleUrls: ['./PlayerHistoryAdvanced.component.scss']
})
export class PlayerHistoryAdvancedComponent implements OnInit,OnDestroy {
  ...
  ngOnInit() {
    this._DataService.UserSession.pipe(takeUntil(this._unsubscribeAll)).subscribe((ServiceUserInformation) => {
      ...
      this.dtOptions = {
        ...
        dom: 'Bfrtip',
        buttons: [
          {extend: 'copy', footer: true, className: 'pi pi-copy', text: '',title:'Player History Advanced'} ,
          {extend: 'print', footer: true, className: 'pi pi-print', text: '',title:'Player History Advanced'} ,
          {extend: 'excel', footer: true, className: 'pi pi-file-excel', text: '',title:'Player History Advanced',filename: 'Player History Advanced '+ this._currentUser.AgentSelected } ,
          {extend: 'pdf'},
          {extend: 'pdfHtml5'},
        ],
  ...
  GetReport() {
    ...
    let info: PlayerHistoryAdvanceRequest = new PlayerHistoryAdvanceRequest();
    ...
    if (this.agentSelected == '' || this.agentSelected == undefined) {
      ...
      info.IdAgent = parseInt(this.agentSelected);
      if (this.playerSelected =='' || this.playerSelected == undefined)
      {
        info.IdPlayer = null;
      }
      else {
        info.IdPlayer = Number(this.playerSelected)
      }
    }
    info.Begin = this._dateRange.Start;
    info.End = this._dateRange.End;


    this._reportService.GetPlayerHistoryAdvanced(info)
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe(data => {
