...
@Component({
  selector: 'app-DaySheetPlus',
  templateUrl: './DaySheetPlus.component.html',
  styleUrls: ['./DaySheetPlus.component.css']
})
export class DaySheetPlusComponent implements OnInit,OnDestroy {
  ...
  public displayModalHightLight: boolean = false;
  public displayModalPlayerNote: boolean=false;
  public displayModalPlayerOnlineMessage: boolean=false;
  ...
  public AgentInformationReturn: any;
  public EditAgentInfomationReturn: number = 2;

  public AgentInformation:any;
  public RowSetting: any;
  public rowcolor: string = '#FFFFFF';
  public ColorList: any[] = [];
  public ColorSetting: any;
  public SaveColorReturn: any[]=[];

  public PlayerNote: any;
  public Note: string = '';
  public SaveNoteReturn: any[]=[];

  public PlayerMessage: any;
  public Message: string = '';
  public MessageList: any[] = [];
  public SavePlayerMessageReturn: any[]=[];
...
/****************************DATATABLE OPTIONS*************************************** */
      this.dtOptions = {
        ...
        dom: 'Bfrtip',
        buttons: [
          {extend: 'copy', footer: true, className: 'pi pi-copy', text: '',title:'Daysheet Plus'} ,
          {extend: 'print', footer: true, className: 'pi pi-print', text: '',title:'Daysheet Plus'} ,
          {extend: 'excel', footer: true, className: 'pi pi-file-excel', text: '',title:'Daysheet Plus',filename: 'Daysheet Plus '+ this._currentUser.AgentSelected + ' ' + this._dateRange.Start} ,
          {extend: 'pdf'},
          {extend: 'pdfHtml5'},
        ],
...
/*************************END DATATABLE OPTIONS*************************************** */
  ...
  GetReport() {
        ...
        let info: DaySheetPlusRequest = new DaySheetPlusRequest();
        ...
        this._reportService.GetDaySheetPlus(info)
          .pipe(takeUntil(this._unsubscribeAll))
          .subscribe(data => {
  ...
  HightlightColorModal(PlayerSelected: any[]){

    this.RowSetting= PlayerSelected;
    this.rowcolor='#'+this.RowSetting.RowColor;

    let info: RowColorListRequest = new RowColorListRequest();

    info.IDAgent = this._currentUser.IdAgentSelected;

    this._reportService.GetDaySheetColorList(info)
    .pipe(takeUntil(this._unsubscribeAll))
    .subscribe(data => {
      this.ColorList = data;
     }, error => {
    });
    this.displayModalHightLight = true;

  }
  PickColor(ColorSelected: any[]){
    this.ColorSetting= ColorSelected;
    this.rowcolor='#'+this.ColorSetting.Color;
  }

  SaveColor(){

    let info: SavePlayerColorRowRequest = new SavePlayerColorRowRequest();

    info.IDAgent =this._currentUser.IdAgentSelected;
    info.IDPlayer= this.RowSetting.ID;
    info.Color= this.rowcolor;

   this._reportService.SavePlayerDaysheetColorRow(info)
    .pipe(takeUntil(this._unsubscribeAll))
    .subscribe(data => {
      this.SaveColorReturn = data;
      this.RowSetting.RowColor=this.rowcolor.replace("#","");
      }, error => {this._loadingReport = false;
    });
    this.displayModalHightLight = false;

  }


  PlayerNoteModal(PlayerSelected: any[]){
    this.PlayerNote= PlayerSelected;
    this.Note =this.PlayerNote.RowNote;

    console.log(this.PlayerNote);

    this.displayModalPlayerNote = true;
  }

  SaveNote(){
    let info: SavePlayerNoteRequest = new SavePlayerNoteRequest();

    info.IDAgent =this._currentUser.IdAgentSelected;
    info.IDPlayer= this.PlayerNote.ID;
    info.Note= this.Note;

   this._reportService.SavePlayerDaysheetNote(info)
    .pipe(takeUntil(this._unsubscribeAll))
    .subscribe(data => {
      this.SaveNoteReturn = data;
      this.PlayerNote.RowNote=this.Note;
      }, error => {this._loadingReport = false;
    });
    this.displayModalPlayerNote = false;

  }


  PlayerOnlineMessageModal(PlayerSelected: any[]){
    this.PlayerMessage= PlayerSelected;

    let info: GetPlayerMessageRequest = new GetPlayerMessageRequest();

    info.IDPlayer = this.PlayerMessage.ID;


   this._reportService.GetPlayerMessages(info)
   .pipe(takeUntil(this._unsubscribeAll))
   .subscribe(data => {
    ...
    this.displayModalPlayerOnlineMessage = true;
  }

  SaveOlineMessage(){
    let info: SavePlayerOnlineMessageRequest = new SavePlayerOnlineMessageRequest();


    info.IDPlayer= this.PlayerMessage.ID;
    info.Message= this.MessageList[0].OnlineMessage;

   this._reportService.SavePlayerDaysheetOnlineMessage(info)
    .pipe(takeUntil(this._unsubscribeAll))
    .subscribe(data => {
      this.SavePlayerMessageReturn = data;


      }, error => {this._loadingReport = false;
    });
    this.displayModalPlayerOnlineMessage = false;
